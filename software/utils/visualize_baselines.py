...
def plot_overall_accuracy(data, ax):
    ...
    bars = ax.bar(
        methods, accuracies, color=colors_list, edgecolor="black", linewidth=1.5
    )
...
def plot_layer_budget_vs_accuracy(data, ax):
    ...
    for acc, label, color, marker in zip(
        naive_accuracies, naive_labels, naive_colors, markers
    ):
        ax.scatter(
            [12],
            [acc],
...
def plot_dynamic_allocations(data, ax):
    ...
    rgb_patch = mpatches.Patch(
        color=COLORS["rgb"], label="RGB Layers", edgecolor="black"
    )
    depth_patch = mpatches.Patch(
...
def main():
    ...
    print(
        f"  • Dynamic 12L achieves {data['dynamic_12']['test_accuracy']:.2f}% with 50% fewer layers than Upper Bound"
    )
    print(
        f"  • Dynamic 8L achieves {data['dynamic_8']['test_accuracy']:.2f}% with only 33% of layers"
    )
    print(
        f"  • Naive Depth (0/12) outperforms Naive RGB (12/0): {data['naive_depth']['test_accuracy']:.2f}% vs {data['naive_rgb']['test_accuracy']:.2f}%"
    )
